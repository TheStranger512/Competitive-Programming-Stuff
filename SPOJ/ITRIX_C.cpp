 ...
 * Created on April 17, 2013, 11:30 PM
 */
...
MinCostFlow<int, llong, 55, 555 > net;
int waits[22][22], ppl[22][22];
int profit[22][22];
...
void solve(int test_num) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    net.addedge(i + size, SINK, 1, 0);
    for (int j = 0; j < size; j++)
      net.addedge(i, j + size, 1, -1LL * min(waits[i][j], ppl[i][j]) * profit[i][j]);
  }
  pair<int, llong> res = net.go(SOURCE, SINK);
  printf("%lld\n", -res.second);
}
