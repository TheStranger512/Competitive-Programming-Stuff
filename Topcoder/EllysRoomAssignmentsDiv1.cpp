...
double EllysRoomAssignmentsDiv1::getAverage(vector <string> ratings) {
  vector<int> vals;
  stringstream ss;
  ss << accumulate(ratings.begin(), ratings.end(), string(""));
  for (int x; ss >> x; )
    vals.push_back(x);
  int me = vals.front();
  int rooms = (vals.size() + 19) / 20, per = vals.size() / rooms;
  int rem = vals.size() % rooms;
  int len = vals.size();
  sort(vals.begin(), vals.end(), greater<int>());
  //pln(vals);
  double res = 0.0;
  for (int r = 0; r + rooms <= len; r += rooms) {
    double avg = accumulate(vals.begin() + r, vals.begin() + r + rooms, 0);
    if (vals[r] >= me && me >= vals[r + rooms - 1])
      res += me;
    else res += avg / rooms;
  }
  res /= per;
  if (rem == 0) return res;
  if (vals[vals.size() - rem] >= me && me >= vals.back())
    return (res * per + me) / (per + 1);
  double rest = accumulate(vals.end() - rem, vals.end(), 0.0) / rem;
  double pExtra = 1.0 * rem / rooms;
  double ans = pExtra * (res * per + rest) / (per + 1) + (1 - pExtra) * res;
  return ans;
}
