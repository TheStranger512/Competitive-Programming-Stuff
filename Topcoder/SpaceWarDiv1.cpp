...
struct SpaceWarDiv1 {
  llong minimalFatigue(vector <int> magicalGirlStrength, vector <int> enemyStrength, vector<llong> enemyCount);
} ;

int str[55];
int good, bad;
pair<int, llong> enemy[55];
llong rem[55];

llong SpaceWarDiv1::minimalFatigue(vector <int> mystr, vector <int> enemystr, vector<llong> cnt) {
  good = mystr.size();
  bad = cnt.size();
  for (int i = 0; i < good; i++)
    str[i] = mystr[i];
  for (int i = 0; i < bad; i++) {
    enemy[i].first = enemystr[i];
    enemy[i].second = cnt[i];
  }
  sort(str, str + good);
  sort(enemy, enemy + bad);
  llong res = 1e17;
  for (llong low = 0, high = llong(1e17); low <= high; ) {
    for (int i = 0; i < bad; i++)
      rem[i] = enemy[i].second;
    llong mid = (low + high) >> 1;
    bool ok = true;
    int at = 0;
    llong fat = 0;
    for (int i = 0; i < bad; ) {
      while (at < good && str[at] < enemy[i].first)
        at++, fat = 0;
      if (at == good) {
        ok = false;
        break;
      }
      llong by = min(mid - fat, rem[i]);
      fat += by;
      rem[i] -= by;
      if (rem[i] == 0) i++;
      if (fat == mid) at++, fat = 0;
  ...
  return res >= llong(1e17) ? -1 : res;
}

// BEGIN KAWIGIEDIT TESTING
// Generated by KawigiEdit 2.1.8 (beta) modified by pivanof

bool KawigiEdit_RunTest(int testNum, vector <int> p0, vector <int> p1, vector<llong> p2, bool hasAnswer, llong p3) {
  cout << "Test " << testNum << ": [" << "{";
