...
int MayTheBestPetWin::calc(vector<int> A, vector<int> B) {
  ...
  //pln(prev[pos(0)]);
  for (int i = 0; i < N; i++) {
//    for (int j = -sum - B[i]; j <= sum + B[i]; j++)
//      cur[pos(j)] = INF;
    memset(cur,0x3F,sizeof(dp[0]));
    sum += B[i];
    for (int j = -sum; j <= sum; j++) {
      chmin(cur[pos(j + B[i])], prev[pos(j)] - A[i]);
      chmin(cur[pos(j - A[i])], prev[pos(j)] + B[i]);
    }
    //pln(sum);
    //pln(prev[pos(0)]);
    //plnarr(cur+pos(-sum+B[i]),cur+pos(sum+B[i]));
    swap(cur, prev);
  }
  //plnarr(prev+pos(-sum), prev+pos(sum));
  int res = INF;
  for (int i = -sum; i <= sum; i++)
    res = min(res, max(i, prev[pos(i)]));
  return res;
}
...
freopen("azn.txt", "w", stderr);
	...
	{
	// ----- test 3 -----
	int t0[] = {6084,1280,593,3104,2452,2901,5316,2641,1315,8260,4893};
			p0.assign(t0, t0 + sizeof(t0) / sizeof(t0[0]));
	int t1[] = {6304,1758,9848,8509,8348,7931,9942,7331,3426,9253,6718};
			p1.assign(t1, t1 + sizeof(t1) / sizeof(t1[0]));
