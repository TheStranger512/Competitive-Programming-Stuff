...
struct KeyDungeonDiv1 {
  int maxKeys(vector<int> doorR, vector<int> doorG, vector<int> roomR, vector<int> roomG,
      vector<int> roomW, vector<int> keys);
};

bitset<1 << 12> dp[121][121]; //whites used for red, green
int cnt[1 << 12][3];

int KeyDungeonDiv1::maxKeys(vector<int> doorR, vector<int> doorG, vector<int> roomR,
  ...
  for (int m = 0; m < (1 << N); m++) {
    ...
    for (int i = 0; i < N; i++) {
      if (1 << i & m) {
        cnt[m][0] += roomR[i] - doorR[i];
        cnt[m][1] += roomG[i] - doorG[i];
        cnt[m][2] += roomW[i];
      }
    }
  }
  ...
  dp[0][0][0] = true;
  for (int m = 0; m < (1 << N); m++) {
    int open = __builtin_popcount(m), haver, haveg, havew, mr, mg;
    for (int r = 10 * open; r >= 0; r--) {
      for (int g = 10 * open; g >= 0; g--) {
        if (!dp[r][g][m])
          continue;
        haver = cnt[m][0] + r;
        haveg = cnt[m][1] + g;
        havew = cnt[m][2] - r - g;
        res = max(res, haver + haveg + havew);
        for (int i = 0; i < N; i++) {
          if (1 << i & m)
            continue;
          mr = max(0, doorR[i] - haver);
          mg = max(0, doorG[i] - haveg);
          if (havew >= mr + mg)
            dp[r + mr][g + mg][m | 1 << i] = true;
        }
      }
    }
  }
...
bool KawigiEdit_RunTest(int testNum, vector<int> p0, vector<int> p1, vector<int> p2, vector<int> p3,
    vector<int> p4, vector<int> p5, bool hasAnswer, int p6) {
  cout << "Test " << testNum << ": [" << "{";
